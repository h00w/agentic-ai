...
autonomy = st.select_slider(
    "Autonomy",
    ["Suggest only", "Approval required", "Low-risk autonomous"],
...
st.code(
    "Goal → Planner → Policy → Tool Gateway → Evidence → Evaluation → Approval → Output → Audit"
)
...
if st.button("Run deterministic lab"):
    ...
    st.json(
        {"goal": goal, "autonomy": autonomy, "trace_events": 6, "external_api_key_required": False}
    )
