...
def release_gate(results: pd.DataFrame, thresholds: GateThresholds) -> tuple[str, dict[str, float]]:
    ...
    metrics = {
        ...
        "high_risk": (
            float(high_risk_rows["passed"].mean()) if not high_risk_rows.empty else 1.0
        ),
    }
    approved = (
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("[Academy](https://hendarmawan.se/agentic-ai/) · [GitHub](https://github.com/h00w/agentic-ai)")
    st.markdown("[Dataset](https://huggingface.co/datasets/h0000w/hendar-agentic-ai-dataset) · [Playground](https://huggingface.co/spaces/h0000w/hendar-agentic-ai)")
...
if gate == "APPROVE":
    st.success("Release gate: APPROVE — all configured quality and safety thresholds are satisfied.")
else:
